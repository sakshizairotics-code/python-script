...
# this line convert hex colour into rgb colour
def hex_to_rgb(hex_color):
    hex_color = str(hex_color).strip().lstrip("#")
    return tuple(int(hex_color[i:i+2], 16) for i in (0, 2, 4))

# this line i can change  text colur
...
def parse_color(value):
    ...
    if v.startswith("#"):
        return hex_to_rgb(v)
    return (205, 105, 180)
# This function wraps long text into multiple lines so that the text does not go outside the screen.
...
# path
def main(csv_path="input/facebook.csv", out_dir="output"):
    os.makedirs(out_dir, exist_ok=True)

    df = pd.read_csv(csv_path)
#this part in csv file
...
# imageclip we used  conveert single image into  a video clip

        clip = ImageClip(frame).with_duration(DURATION).with_fps(FPS)
#start with  saving video  file to  disk
